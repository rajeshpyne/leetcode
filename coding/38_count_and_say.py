...
"""
Approach :-
1. Create a dictionary where we store all the digits with words.
    {"one":1,"two":2,"three":3,....,"nine":9}
2. count consecutive number from the beginning and store the count number after splitting
3. 
"""

class Solution:
    def consecutiveNum(self, n : int)->int:
        num_dict = {}
        num_list = ([int(a) for a in str(n)])
        count = 0
        for a in range(0,len(num_list)-1):
            # print(num_list[a])
            if num_list[a] == num_list[a+1]:
                num_dict.update({num_list[a]:count+1})
            else:
                count = 1
                num_dict.update({num_list[a]:count})
        print(num_dict)
